...
from herrramientas_de_entrenamiento_i9_gen_gray import train_for_classification
...
from generador_de_transformaciones_v2 import make_transformation_selection
...
# Generador precalculador de transformaciones
from json_generator_gray_v2 import produce_reductions_json
...
def run_it(dimension, delta_alfa, red, input_folder):
  ...
  input_folder = f'F_PairedEpochs_Adaptive_{input_folder}'

  NEURAL_NETWORK_NAME = red # [STAR] Tiene una ocurrencia abajo, en caso de cambiarla, cambiar ambas
  FOLDER_EXPERIMENT = f'New_PairedEpochs_Adaptive/{input_folder}/{NEURAL_NETWORK_NAME}/{DIMENSION}/{D_ALFA}'
  ...
  gen_train_loader = DataLoader(GenDataset(transform=transform, list='humanet_training_set_v3_f.json'), batch_size=BATCH_SIZE,
                            shuffle=True, num_workers=8)
  ...
  for i in range(CICLOS):
    ...
    # Generar las reducciones
    gen_time = produce_reductions_json(net, gen_train_loader, d_alpha, dimension)
    reduction_generation_time.append(gen_time)
    # Generar loader con modo reduccion
    reduction_train_loader = DataLoader(MEPIDataset(transform=transform, list='humanet_training_set_v2.json'), batch_size=BATCH_SIZE,
                            shuffle=True, num_workers=8)
    ...
    # Epocas Reducidas
      ...
      train_loss, acc, e_time = train_for_classification(net, reduction_train_loader, 
                                                test_loader, optimizer, 
                                                criterion, lr_scheduler=scheduler, 
                                                epochs=1, reports_every=REPORTS_EVERY, device='cuda',
                                                reducir=False, reduccion=True, delta_alpha=d_alpha)
    ...
    if acc_val_anterior == acc_val_actual:
      ...
      print(f'No es necesario seguir')
      early_stopper_watcher = early_stopping_num
  ...
  persist_list(reduction_generation_time, f'{FOLDER_TRAINING_REPORTS}/ReductionGenerationTimes')
